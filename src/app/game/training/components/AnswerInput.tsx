...
interface AnswerInputProps {
    selectedWaits: HaiKindId[];
    onToggleWait: (tile: HaiKindId) => void;
    isPortrait: boolean;
}

export function AnswerInput({ selectedWaits, onToggleWait, isPortrait }: AnswerInputProps) {
    const answerOptions = [
    ...
    return (
        <View className="px-4 items-center mb-10">
            <Text className="text-gray-400 mb-4 text-base">待ち牌をすべて選択してください</Text>
            <View
                className="flex-row justify-center gap-3"
                style={isPortrait ? { width: PORTRAIT_GRID_WIDTH, flexWrap: 'wrap' } : { width: 'auto', flexWrap: 'nowrap' }}
            >
